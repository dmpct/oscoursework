...
namespace Editor {
    ...
    const char* C_HL_extensions[] = { ".p", nullptr };
    const char* C_HL_keywords[] = {
        "cal|",      "input|",   "print|", "read|", "write|",
        "mem|", "pri|", "ret|",   nullptr };
    ...
    fg editorSyntaxToColor(int hl) {
        ...
        case HL_COMMENT:
        case HL_MLCOMMENT:
            return fg::bright_green;
        case HL_KEYWORD1:
            return fg::bright_yellow;
        case HL_KEYWORD2:
            return fg::bright_yellow;
        case HL_STRING:
    ...
    void editorOpen(char* filename, std::function<void(std::string, char*)> fn) {
        ...
        char* buf = new char[15 * 1024];
        fn(std::string(filename), buf);
        std::string data(buf);
        std::string line;
        delete buf;
        size_t pos = 0;
    ...
    void editorSave(const Terminal& term, std::function<void(std::string, char*, size_t)> fn) {
        if (E.filename == nullptr) {
    ...
    void editor(Terminal term, std::string file,
        ...
        editorRefreshScreen();
        while (editorProcessKeypress(term, ofn, sfn)) {
            editorRefreshScreen();
        }
        term.restore_screen();
    }
